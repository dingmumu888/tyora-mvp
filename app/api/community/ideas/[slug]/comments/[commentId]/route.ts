...
import { deleteCommunityCommentOwner, toggleCommunityCommentReaction } from "@/lib/server/community-store";
...
export async function PATCH(_request: Request, { params }: { params: Promise<{ slug: string; commentId: string }> }) {
  ...
  const { slug, commentId } = await params;
  try {
    return refreshCommunitySessionCookieIfNeeded(ok(await toggleCommunityCommentReaction(slug, commentId, session.userId)), session);
  } catch (error) {
