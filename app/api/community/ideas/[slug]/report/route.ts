...
import { getCommunityIdeaBySlug, reportCommunityIdea } from "@/lib/server/community-store";
...
export async function POST(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!session) {
    return await getCommunityIdeaBySlug(slug)
      ? fail("Email login is required to report a discussion.", 401)
      : fail("Not found.", 404);
  }
  try {
    const body = await request.json() as { reason?: string };
    const data = await reportCommunityIdea(
      slug,
      body.reason,
      session.userId,
      request,
    ...
    return fail(messageFromError(error, "Unable to report discussion."), 400);
  }
}
